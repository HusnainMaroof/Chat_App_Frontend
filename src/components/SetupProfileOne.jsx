...
import { CheckCircle2, ChevronDown, FileText, Globe } from "lucide-react";
          ...
          <AnimatePresence>
            {isOpen && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute z-50 w-full bg-slate-900 border border-slate-700 shadow-2xl rounded-xl mt-1 max-h-60 overflow-y-auto custom-scrollbar"
              >
                {Countries.map((item) => (
                  <div
                    key={item.country}
